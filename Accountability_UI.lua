local A = Apollo.GetAddon("Accountability")
...
function A:InitWindow()
	if self.xmlDoc ~= nil and self.xmlDoc:IsLoaded() then
		self.wndMain = Apollo.LoadForm(A.xmlDoc, "AccountabilityForm", nil, A)
...
function A:UpdateUI()
	...
	elseif self.settings.viewMode == 2 then
		...
		self:UpdateCurrentTeamView()
	end
end

function A:UpdateGroupView()
	self:UpdateRows()
...
function A:UpdateTeamView()
	
end

function A:UpdateCurrentTeamView()

end

function A:ClearRowView()
	self:ClearRows()
end

--Returns players position, name and color based on current group table information
...
--Returns a table for each team with the following information
--NOTE: Returns them sorted by team's total IA
--    {
--        name = <team name>
--        totalIA = <all available IA in team added together, nil if no members>
--		  members (nil if no members) = {
--			  [<index for each members name>] = <member's total IA>
--		  }		
--    }
function A:TeamTableIterator()
	local temp = {}
	
	--Do nothing if no teams
	if tablelength(self.teamList) == 0 then
		return nil
	end
	
	for teamName in pairs(self.teamList) do
		local teamTable = {}
		teamTable.members = {}
		local names = {}
		local IA = nil
		...
		for playerName, player in pairs(self.group) do
			if self.teams[playerName] and self.teams[playerName] == teamName then
				if player.totalIA then
					teamTable.members[playerName] = player.totalIA
					IA = IA and IA + player.totalIA or player.totalIA
				else
					--Cant be nil or they wont exist
					teamTable.members[playerName] = -1
				end
			end
		end
		
		if IA then
			teamTable.totalIA = IA
		end
		
		temp[#temp + 1] = teamTable
	end
	
	table.sort(temp, function(a,b) 
		if not a.totalIA and not b.totalIA then
			--doesnt matter
			return true
		end
		if not a.totalIA and b.totalIA then
			return false
		end
		if not b.totalIA and a.totalIA then
			return true
		end
		return a.totalIA < b.totalIA
	end)
...
function A:ChangeViewMode()
	if self.settings.viewMode == 4 then
		self.settings.viewMode = 1
	else
		self.settings.viewMode = self.settings.viewMode + 1
	end
	...
	elseif self.settings.viewMode == 2 then
		self.wndMain:FindChild("Header"):SetText("       Raid Interrupts By Team (TBI)")
	elseif self.settings.viewMode == 3 then
		self.wndMain:FindChild("Header"):SetText("       Raid Interrupts By Team (Row)(TBI)")
	else
		self.wndMain:FindChild("Header"):SetText("       Current Interrupt Team (TBI)")
	end
